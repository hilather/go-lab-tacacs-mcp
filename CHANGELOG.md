...
- PEAP is opt-in `peap` only ([ADR 0030](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0030-radius-peap-tls-in-eap-start.md)): Identity issues PEAP Start, then a TLS 1.3 tunnel carries inner PEAPv0 EAP-MSCHAPv2. Not PEAPv1/GTC, PEAP-EAP-TLS, crypto-binding, or Windows interop. Type 25 without `peap` still fail-closes. No EAP-TLS / EAP-TTLS / TEAP / pass-through. A PEAP TLS server certificate must be injected on the access handler (tests do); TacLab does not mint ephemeral PEAP certs.
...
- Opt-in `peap` issues EAP-Request/PEAP Start (type 25, RFC 5216 S flag, PEAPv0) after Identity, then a consume-on-use TLS-in-EAP pump completes a server-authenticated TLS 1.3 tunnel and an inner EAP-MSCHAPv2 conversation via the same AAA path as RADIUS MS-CHAP ([ADR 0030](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0030-radius-peap-tls-in-eap-start.md)). Independent `testclient` UDP evidence covers Start, ClientHello→server TLS records, and Accept. `PRJ-EAP-003` stays `DEFERRED_MAY`.
