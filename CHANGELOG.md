...
- PEAP is **Start only** ([ADR 0030](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0030-radius-peap-tls-in-eap-start.md)): opt-in `peap` issues EAP-Request/PEAP Start (type 25, S flag) after Identity. TLS-in-EAP framing and a TLS 1.3 server exist; handshake continuation and inner EAP are not implemented. Type 25 without `peap` still fail-closes. No EAP-TLS / EAP-TTLS / TEAP / pass-through.
...
- Opt-in `peap` issues EAP-Request/PEAP Start (type 25, RFC 5216 S flag, PEAPv0) after Identity. `internal/radius/eap/peap` frames L/M/S and runs a server-authenticated TLS 1.3 handshake in tests (`NewServer`, `HandshakeWithClient`). Continuation after Start still Rejects — not complete PEAPv0/EAP-MSCHAPv2 ([ADR 0030](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0030-radius-peap-tls-in-eap-start.md)). `PRJ-EAP-003` stays `DEFERRED_MAY`.
