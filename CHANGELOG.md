...
- Opt-in `peap` issues EAP-Request/PEAP Start (type 25, RFC 5216 S flag, PEAPv0) after Identity, then a consume-on-use TLS-in-EAP pump completes a server-authenticated TLS 1.3 tunnel and an inner EAP-MSCHAPv2 conversation via the same AAA path as RADIUS MS-CHAP ([ADR 0030](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0030-radius-peap-tls-in-eap-start.md)). `taclabd` attaches PEAP from the default RadSec or SecureTACACS TLS identity (no ephemeral mint). Independent `testclient` UDP evidence covers Start, ClientHello→server TLS records, and Accept. `PRJ-EAP-003` stays `DEFERRED_MAY`.
