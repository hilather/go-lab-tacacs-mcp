...
RADIUS diagnostics are distinct from the TACACS `authentication.test` / `policy.evaluate` ops (KD-14). `radius.access.test` calls the same `AuthenticateAccess` path as UDP for PAP, CHAP, opt-in MS-CHAP, and one-shot EAP-MD5 (`method.type` is `pap`, `chap`, `mschapv1`, `mschapv2`, or `eap`; passwords, MS-CHAP material, and EAP challenge/response are write-only and wiped). EAP Identity (type `eap` without challenge/response) returns `outcome=access_challenge` with `state_present: true` only — never raw State and never EAP-Message. `radius.policy.evaluate` uses the compiled RADIUS engine and accepts the same method tokens. `radius.attributes.list` returns dictionary metadata only (name/code/vendor/value_kind/allowed_in/sensitivity/source). `source` is `builtin` or `operator:<id>`. These do not advertise complete RADIUS.
...
- Tool input/output schemas are derived from the same Go request/response types used by REST. Framing uses `github.com/modelcontextprotocol/go-sdk` v1.7.0 ([ADR 0011](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0011-mcp-thin-adapter-go-124.md)).
