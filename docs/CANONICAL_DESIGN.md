...
| EAP | Identity + EAP-MD5 terminate when `eap` is opted in. Opt-in `peap` issues PEAP Start (type 25) after Identity; handshake continuation and inner EAP are not implemented ([ADR 0030](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0030-radius-peap-tls-in-eap-start.md)). Type 25 without `peap` → generic EAP-Failure + Access-Reject. Complete tunneled EAP stays deferred (`PRJ-EAP-003`, [ADR 0022](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0022-radius-eap-identity-md5.md)). |
