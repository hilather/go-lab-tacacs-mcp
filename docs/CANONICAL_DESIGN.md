...
| RADIUS design | [docs/designs/radius-authentication.md](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/designs/radius-authentication.md) (MVP). Remaining work: [docs/designs/radius-remaining-work.md](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/designs/radius-remaining-work.md) |
| RADIUS ADRs | [0013](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0013-add-radius-to-existing-taclab-process.md)–[0018](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0018-preserve-product-and-module-names-for-first-radius-release.md) (shipped MVP). Remaining-work program: [0020](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0020-in-memory-radius-remaining-work-program.md)–[0029](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0029-user-group-radius-policy-attachment.md). PEAP Start: [0030](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0030-radius-peap-tls-in-eap-start.md). Must-change: [0019](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0019-force-password-change.md) |
...
These limits are product contract, not temporary stubs. Operator text lives in [docs/OPERATOR.md](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/OPERATOR.md) §1.1. Remaining-work names and sequencing live in [docs/designs/radius-remaining-work.md](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/designs/radius-remaining-work.md); ADRs [0020](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0020-in-memory-radius-remaining-work-program.md)–[0029](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0029-user-group-radius-policy-attachment.md) are accepted and the matching implementation PRs attach evidence. [ADR 0030](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0030-radius-peap-tls-in-eap-start.md) starts opt-in PEAP Start only. Residuals that **remain**: **no persist**, **no complete PEAP/EAP-TLS**, **no DTLS/RADIUS/1.1**, **no proxy**, and `conformance_status` stays **`partial`**.

| Residual | Binding |
|---|---|
| Lab appliance | Single replica. Overlay, RADIUS cache, accounting journal, event ring, Challenge store, and CoA session index are process memory. Restart or `runtime.reset` restores the YAML baseline ([ADR 0020](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0020-in-memory-radius-remaining-work-program.md)). |
| UDP is the default RADIUS profile | RADIUS/UDP is MD5-era, spoofable, and mostly cleartext ([ADR 0016](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0016-radius-udp-security-retransmission-and-scope.md)). Keep 1812/1813/3799 off the public internet. Optional RadSec (RFC 6614 TLS 1.3 TCP 2083) is a stream listener, default off ([ADR 0025](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0025-radius-radsec-tls13-first-slice.md)). It is not “UDP plus TLS.” DTLS/RADIUS/1.1 stay deferred. Keep 2083 off the public internet unless published like TACACS 300. |
| Access-Challenge | `R65-ACCESS-004` is `PASS` with independent testclient wire evidence. In-memory State store: UDP IP + TLS cert bind, TTL, consume-on-use, capacity fail-closed. Program ADR [0021](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0021-radius-access-challenge-state-gate.md). |
| EAP | Identity + EAP-MD5 terminate when `eap` is opted in. Opt-in `peap` issues PEAP Start then TLS 1.3 + inner PEAPv0 EAP-MSCHAPv2 ([ADR 0030](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0030-radius-peap-tls-in-eap-start.md)). `taclabd` reuses the default RadSec or SecureTACACS TLS identity and does not mint ephemeral PEAP certificates. Type 25 without `peap` → generic EAP-Failure + Access-Reject. Complete tunneled EAP (PEAPv1, EAP-TLS, crypto-binding, Windows interop) stays deferred (`PRJ-EAP-003`, [ADR 0022](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0022-radius-eap-identity-md5.md)). |
| CoA / Disconnect | DAC originate is shipped (`PRJ-COA-001`). Optional inbound DAS (`PRJ-COA-002`, UDP 3799, default off) is an RFC 5176 echo fixture: index-only, does not kick a NAS. DAC uses the client's UDP RADIUS secret ([ADR 0024](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0024-radius-coa-disconnect.md)). |
...
| Named `Cisco-AVPair` | Shipped (vendor 9, vendor-type 1). Evidence is independent `testclient` fixtures ([ADR 0027](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0027-named-cisco-avpair-independent-fixtures.md)). An IOL skip is not PASS. Other Cisco vendor-types stay raw. |
...
| Advertisement | `system.build.get` RADIUS `conformance_status` stays `partial` for this entire program. Do not market “complete RADIUS.” |
...
- Complete RADIUS tunneled EAP (PEAPv0/inner EAP, EAP-TLS, TTLS), Access-Challenge as an advertised completeness feature, DTLS/RADIUS/1.1, proxying, persistent RADIUS accounting, or a second daemon/module rename. Opt-in PEAP Start is not complete PEAP. Named `Cisco-AVPair` (vendor 9 type 1), RADIUS MS-CHAP, operator dictionaries, and optional RadSec (TLS 1.3 TCP 2083, default off) are remaining-work slices, not completeness claims. Inbound DAS is an index-only echo fixture and does not kick a NAS.
...
| ENABLE | 0 | multi-step like ASCII | `action=LOGIN`, `authen_service=ENABLE`; **ignore `authen_type`** (ASCII or PAP in START still runs ENABLE). Goldens required. `must_change_login` does **not** apply to ENABLE. After a **successful** enable verify, if `must_change_enable`, TacLab **may** continue with extra GETPASS new/confirm (`New Password: ` / `Retype New Password: `). This is a **lab/vendor extension**, not RFC 8907 §5.4.2.6 ENABLE. Gated only by `enable` already being allowed; there is no `enable_chpass`. |
