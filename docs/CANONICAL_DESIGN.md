...
| EAP | Identity + EAP-MD5 terminate when `eap` is opted in. Other types → generic EAP-Failure + Access-Reject. Tunneled EAP stays deferred ([ADR 0022](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0022-radius-eap-identity-md5.md)). |
...
- RADIUS tunneled EAP, Access-Challenge as an advertised completeness feature, DTLS/RADIUS/1.1, proxying, persistent RADIUS accounting, or a second daemon/module rename. Named `Cisco-AVPair` (vendor 9 type 1), RADIUS MS-CHAP, operator dictionaries, and optional RadSec (TLS 1.3 TCP 2083, default off) are remaining-work slices, not completeness claims. Inbound DAS is an index-only echo fixture and does not kick a NAS.
