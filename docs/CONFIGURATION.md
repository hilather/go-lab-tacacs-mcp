...
| `2` | `listeners.tacacs.legacy`, `listeners.tacacs.tls`, `listeners.radius.{access,accounting,radsec,dynamic_authorization}`, `listeners.http` | Parsed directly. RADIUS listener structs are compiled (defaults `enabled: false`). |
...
RADIUS listeners default `enabled: false`. When enabled they bind UDP on a controlled lab network. Access-Request is PAP/CHAP plus compiled-policy Access-Accept/Reject (Message-Authenticator first). Accounting Start/Stop/Interim/On/Off writes the memory ring and replies Accounting-Response only after the ring accepts the record. Optional `listeners.radius.dynamic_authorization` (UDP 3799, default off) is an RFC 5176 echo fixture: ACK/NAK and session-index mutation only. It does not kick a NAS. `require_message_authenticator` cannot be disabled. Operators who want inbound packets add `dynamic_authorization` to the client RADIUS endpoint `roles`. Optional `listeners.radius.radsec` is a TLS 1.3 stream on TCP 2083, default off. TACACS and RADIUS listeners start through `internal/runtime.Registry`. Process start requires at least one AAA listener unless `server.admin_only: true`. Dynauth-only is not an AAA listener. Do not advertise RADIUS completeness. Residual limits (no EAP termination advertised as complete, inbound DAS default-off echo, RadSec default-off, or persistent accounting) are in [OPERATOR.md](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/OPERATOR.md) §1.1 and [CANONICAL_DESIGN.md](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/CANONICAL_DESIGN.md) residual limits. Named `Cisco-AVPair` is accepted on reply profiles. DAC CoA/Disconnect originate is available (`radius:dynamic`); handle path needs Accounting-Start. Accounting listener knobs `session_index_entries` (default 20000), `session_index_bytes` (8MiB), `session_ttl` (24h), and `coa_timeout` (3s) cap the memory index. Client UDP endpoint optional `coa_destination` / `nas_coa_port` (default 3799) select the NAS DAC address. The checked-in combined example is [configs/lab.example.v2.yaml](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/configs/lab.example.v2.yaml).
