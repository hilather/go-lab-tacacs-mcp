...
RADIUS listeners default `enabled: false`. When enabled they bind UDP (compiled `RADIUSIndex` for source match). Access is still a structural Access-Reject stub (not PAP/CHAP). Accounting records start/stop/interim/on/off through the event ring after authenticator checks; inbound Message-Authenticator is validate-if-present. TACACS and RADIUS listeners start through `internal/runtime.Registry`. Process start requires at least one AAA listener unless `server.admin_only: true`. Do not advertise RADIUS completeness.
