...
| Access-Challenge / EAP | Identity (type 1) + EAP-MD5 (type 4) terminate when `allowed_authentication_methods` includes `eap` (opt-in; omitted lists stay `[pap, chap]`). Opt-in `peap` issues PEAP Start then a TLS 1.3 tunnel with inner PEAPv0 EAP-MSCHAPv2. PEAP reuses the default RadSec TLS identity, then the SecureTACACS default (the listener may stay disabled). TacLab does **not** mint ephemeral PEAP certificates; `peap` without a configured identity fail-closes. Not PEAPv1/GTC / PEAP-EAP-TLS / crypto-binding / Windows interop. Type 25 without `peap` still gets generic EAP-Failure + Access-Reject. No EAP-TLS/TTLS/TEAP. `must_change_login` after a good MD5 is Access-Reject + the same generic EAP-Failure as a bad password. Program ADRs [0021](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0021-radius-access-challenge-state-gate.md) / [0022](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0022-radius-eap-identity-md5.md) / [0030](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0030-radius-peap-tls-in-eap-start.md). |
...
| Item | Behavior |
|---|---|
| Access | PAP, CHAP, and opt-in MS-CHAPv1/v2 / EAP / PEAP Start (`mschapv1` / `mschapv2` / `eap` / `peap` on `allowed_authentication_methods`; omitted lists stay `[pap, chap]`). Access-Accept, Access-Reject, or Access-Challenge (EAP Identity or PEAP Start) after integrity + known client. Must-change is Access-Reject with no `MS-CHAP-Error`. PEAP Start is not a complete PEAPv0 login. |
