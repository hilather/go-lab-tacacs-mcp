...
| PRJ-RADSEC-001 | PROJECT MUST | RADIUS/TLS 1.3 stream on TCP 2083 (mTLS, length-prefixed, secret required) | Independent `testclient` TLS client; PAP/CHAP only until EAP/MS-CHAP land; [ADR 0025](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0025-radius-radsec-tls13-first-slice.md) | [x] |
...
ADRs [0020](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0020-in-memory-radius-remaining-work-program.md)–[0029](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0029-user-group-radius-policy-attachment.md) are accepted. Operator dictionaries (`RAD-EXT-006` / `PRJ-DICT-001`) are implemented. MS-CHAP is `PASS` as `PRJ-MSCHAP-001`. User/group RADIUS attachment is `PASS` as `PRJ-POL-002`. Remaining Challenge/EAP/CoA/RadSec/Cisco-AVPair rows stay `DEFERRED_MAY` / not started until their implementation PR attaches independent `internal/radius/testclient` evidence where required. Do not invent `R3579-EAP-*` or `R5080-*` IDs. Do not flip `R65-ACCESS-004` to `PASS` from documentation alone. `system.build.get` RADIUS `conformance_status` stays `partial` even after this program's in-scope rows land ([ADR 0020](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0020-in-memory-radius-remaining-work-program.md)).
...
| UDP remains the default lab profile | RADIUS/UDP is still MD5-era and mostly cleartext. RadSec (RFC 6614 TLS 1.3 TCP 2083) is an **optional** stream listener, default off. It is not “UDP plus TLS.” DTLS/1.1 stay deferred. Keep **2083** off the public internet unless intentionally published like TACACS 300 |
...
| Inbound CoA / Disconnect deferred | DAC originate is shipped. Inbound :3799 DAS is still out (echo fixture only, default off). DAC uses the UDP endpoint secret ([ADR 0024](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0024-radius-coa-disconnect.md)) |
