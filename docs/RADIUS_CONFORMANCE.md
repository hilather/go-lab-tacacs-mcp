...
| Tunneled EAP (PEAP/TLS/TTLS) / pass-through | `RAD-EXT-002` residual / `PRJ-EAP-003` | `DEFERRED_MAY` | [0022](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0022-radius-eap-identity-md5.md) | complete tunneled method later |
| PEAP Start (outer type 25 + TLS-in-EAP) | `RAD-PEAP-001` | Start increment | [0030](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0030-radius-peap-tls-in-eap-start.md) | Identity→PEAP Start when `peap` opted in |
| PEAP handshake + inner EAP-MSCHAPv2 | `RAD-PEAP-002` | Lab increment — not complete tunneled EAP | [0030](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0030-radius-peap-tls-in-eap-start.md) | TLS 1.3 pump + inner MSCHAPv2; `PRJ-EAP-003` stays `DEFERRED_MAY` |
...
| RADIUS proxying / realm routing | `RAD-EXT-008` / `PRJ-PROXY-001` | `DEFERRED_MAY` | [0028](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0028-defer-radius-proxying.md) | out of this program; no `proxy` YAML key |
| Persistent accounting | `RAD-EXT-009` / `PRJ-ACCT-003` | **cancelled this program** | [0020](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0020-in-memory-radius-remaining-work-program.md) | later persist ADR only |
...
| EAP | Identity + EAP-MD5 terminate. Opt-in PEAP Start + TLS 1.3 + inner PEAPv0 EAP-MSCHAPv2 when a RadSec or SecureTACACS identity exists (no ephemeral mint). Other types → generic EAP-Failure + Access-Reject. Complete tunneled EAP stays `DEFERRED_MAY` (`PRJ-EAP-003`) |
