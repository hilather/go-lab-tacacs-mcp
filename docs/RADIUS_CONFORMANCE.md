...
ADRs [0020](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0020-in-memory-radius-remaining-work-program.md)–[0029](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0029-user-group-radius-policy-attachment.md) are accepted. Rows below stay `DEFERRED_MAY` / not started until the matching implementation PR attaches independent `internal/radius/testclient` evidence (MS-CHAP is `PASS` as `PRJ-MSCHAP-001`; user/group RADIUS attachment is `PASS` as `PRJ-POL-002`). Do not invent `R3579-EAP-*` or `R5080-*` IDs. Do not flip `R65-ACCESS-004` to `PASS` from documentation alone. `system.build.get` RADIUS `conformance_status` stays `partial` even after this program's in-scope rows land ([ADR 0020](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0020-in-memory-radius-remaining-work-program.md)).
