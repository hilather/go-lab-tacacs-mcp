...
- [x] Listener registry (`internal/runtime`) wraps TACACS and RADIUS listeners. `secretLookup` handles `RADIUSSharedSecret`. `system.status.get` overlays live Runtime stats and lists configured RADIUS sockets.
