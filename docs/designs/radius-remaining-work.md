...
| Config v2 | `internal/config/raw_v2.go`, `types.go` | Named listeners including optional `dynamic_authorization` and `radsec`. At most one RADIUS UDP endpoint and one RADIUS TLS endpoint per client. v2 `users[]` / `groups[]` accept `radius_policy_id`. |
| Runtime IDs | `internal/runtime/listener.go` | `IDRADIUSAccess`, `IDRADIUSAccounting`, `IDRADIUSRadSec`, `IDRADIUSDynAuth` (optional inbound DAS). |
...
| KD-R05 | EAP **termination** is Identity (type 1) + EAP-MD5 (type 4) when `eap` is opted in. Opt-in `peap` issues PEAP Start (type 25, S flag) after Identity; handshake continuation and inner EAP stay fail-closed ([ADR 0030](https://github.com/hilather/go-lab-tacacs-mcp/blob/main/docs/decisions/0030-radius-peap-tls-in-eap-start.md)). Type 25 without `peap` still fail-closes. No EAP-TLS/TTLS/pass-through. | Start-only PEAP is opt-in so existing `eap` labs keep Identity+MD5. |
...
| KD-R21 | Compile default for omitted/empty `allowed_authentication_methods` on an access role stays **`[pap, chap]`** (`normalizeRADIUSEndpoint` today). `eap`, `peap`, `mschapv1`, and `mschapv2` are **opt-in**. Runtime `methodAllowed` empty-slice = allow-all is defensive only and must not be reachable after compile. | Expanding the default would silently enable MS-CHAP/EAP/PEAP on every existing v2 client that omitted the list. |
...
- If EAP-Identity is empty and User-Name is present, use User-Name. If `peap` is allowed, proceed to PEAP Start; else proceed to MD5-Challenge.
...
| `clients.*` | `allowed_authentication_methods` may include `eap`, `peap`, `mschapv1`, `mschapv2` (opt-in; omitted still compiles to pap+chap). Optional `roles` include `dynamic_authorization`. Optional second endpoint `transport: tls`. Optional `nas_coa_port`, `coa_destination`. |
...
| RAD-TM-22 | EAP type confusion / tunneled downgrade | High | Unknown types fail closed; type 25 without `peap` still fail-closes; PEAP Start is opt-in and continuation Rejects | PRJ-EAP-002; `TestIndependentTestclientPEAPStartWire` |
