...
func Generate(opts Options) (*Result, error) {
	...
	if err := os.WriteFile(filepath.Join(cfgDir, "taclab.yaml"), []byte(dual), 0o644); err != nil {
		return nil, err
	}
	if err := os.WriteFile(filepath.Join(cfgDir, "taclab.tls-only.yaml"), []byte(tlsOnly), 0o644); err != nil {
		return nil, err
	}
	...
	man := Manifest{
		...
		SourceIPNote:            "Compose published ports may NAT the TACACS peer. Use host network or macvlan before claiming device-accurate client match (LAB_DEPLOYMENT §4.3). Match key is the TCP peer, never X-Forwarded-For.",
...
func prepareDir(dir string, force bool) error {
	...
	for _, rel := range []string{"secrets", "certs-public", "pki", "config/taclab.yaml", "config/taclab.tls-only.yaml", "manifest.json"} {
...
func validateGenerated(dir string) error {
	path := filepath.Join(dir, "config", "taclab.yaml")
	doc, err := config.Load(path)
	if err != nil {
		return err
	}
	return config.Validate(doc)
}
...
Dual-listener (host 49 / 300 / 8080):
...
Source-IP: published ports may NAT the TACACS peer. See
